...
public class JobAdDAOESClient implements IJobAdDAO {
	...
	private static Logger logger = LoggerFactory
			.getLogger(JobAdDAOESClient.class);

	public JobAd getById(String id) throws Exception {
		JobAd ad = null;
		Get adGet = new Get.Builder(Constants.KAAMHAI_INDEX, id).type(
				Constants.JOBAD_TYPE).build();
		JestResult result = ESClient.getInstance().execute(adGet);
		if (result.isSucceeded()) {
			ad = result.getSourceAsObject(JobAd.class);
		} else {
			logger.error("Exception occured while find ad with id {}",
	...
	public List<JobAd> get() throws Exception {
		...
		bldr.size(1000);
		Search search = new Search.Builder(bldr.toString())
				.addIndex(Constants.KAAMHAI_INDEX)
				.addType(Constants.JOBAD_TYPE).build();
	...
	public JobAd create(JobAd jobAd) throws Exception {
		String response = ElasticSearchClient.create(Constants.JOBAD_TYPE,
				JSONSerializer.generateJSON(jobAd));
		JSONObject result = new JSONObject(response);
		String id = result.getString("_id");
		jobAd.setId(id);
		response = ElasticSearchClient.update(Constants.JOBAD_TYPE,
				JSONSerializer.generateJSON(jobAd), id);

		response = ElasticSearchClient.getById(Constants.JOBAD_TYPE, id);
		result = new JSONObject(response);
		JSONObject jobAdResult = result.getJSONObject("_source");

		JobAd jobAdspective = (JobAd) JSONDeserializer.generate("JobAd",
				jobAdResult.toString());
		return jobAdspective;
	}

	public JobAd update(JobAd jobAd, String id) throws Exception {
		String response = ElasticSearchClient.update(Constants.JOBAD_TYPE,
				JSONSerializer.generateJSON(jobAd), id);
	...
	@Override
	public List<JobAd> search(String gender, String location, String city,
			String language, String category) throws Exception {
		
		SearchSourceBuilder bldr = new SearchSourceBuilder();
		BoolFilterBuilder filterBuilder = FilterBuilders.boolFilter();
		if (gender != null) {
			filterBuilder.should(FilterBuilders.prefixFilter("gender",
					gender.toLowerCase()));
		}
		if (location != null) {
			filterBuilder.should(FilterBuilders.prefixFilter("location",
					location.toLowerCase()));
		}
		if (city != null) {
			filterBuilder.must(FilterBuilders.prefixFilter("city",
					city.toLowerCase()));
		}
		if (language != null) {
			filterBuilder.should(FilterBuilders.prefixFilter("language",
					language.toLowerCase()));
		}
		if (category != null) {
			filterBuilder.must(FilterBuilders.prefixFilter("category",
	...
	@Override
	public List<JobAd> search(String searchString)
			throws ClientProtocolException, IOException, Exception {
		SearchSourceBuilder bldr = new SearchSourceBuilder();
		MultiMatchQueryBuilder qbldr = QueryBuilders.multiMatchQuery(
				searchString.toLowerCase().split(" "), "location", "city",
				"category");
		bldr.query(qbldr);
		bldr.size(100);
		return getJobAds(bldr);
	...
	private List<JobAd> getJobAds(SearchSourceBuilder bldr)
			throws ClientProtocolException, IOException, Exception {
		logger.info(bldr.toString());
		List<JobAd> ads = new ArrayList<>();
		String response = ElasticSearchClient.search(Constants.JOBAD_TYPE,
				bldr.toString());
		JSONObject result = new JSONObject(response);
		...
		for (int i = 0; i < hits.length(); i++) {
			...
			JobAd jobAd = (JobAd) JSONDeserializer.generate("JobAd",
					source.toString());
			ads.add(jobAd);
		}
		return ads;
